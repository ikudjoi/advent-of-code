...
class Monkey:
    ...
    def inspect(self):
        for old in self.items:
            ...
            # use least common multiple to cap the values reasonable
            # does not affect inspection counts!
            new = new % self.least_common_multiple
            div_rest = new % self.divisible_by
            if div_rest == 0:
                yield self.true_target_monkey, new
            else:
                yield self.false_target_monkey, new
...
divisors = [m.divisible_by for m in monkeys]
